    ...
    <div v-else>
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          ...
          >
            <!-- Regular Item Card Content ... (omitted for brevity, keeping same logic) -->
      ...
      <!-- Seasonal Items Section (Moved to Bottom and Collapsed) -->
      <div v-if="seasonalItems.length > 0" class="mt-16 pt-8 border-t border-zinc-200 dark:border-white/5">
        <button 
          @click="showSeasonal = !showSeasonal"
          class="w-full flex items-center justify-between p-6 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-white/5 hover:border-amber-500/30 transition-all group"
        >
          <div class="flex items-center gap-4">
            ...
            <div class="text-left">
              <h2 class="text-xl font-black uppercase tracking-tight text-zinc-900 dark:text-white italic">Objetos de Temporada</h2>
              <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium">Recompensas especiales y de roadmap.</p>
            </div>
          </div>
          <ChevronDown 
        ...
        >
          <div v-if="showSeasonal" class="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-top-4">
            <div 
...
import { LayoutGrid, Type, Frame, Sparkles, ChevronDown } from 'lucide-vue-next';
...
const categories = [
  ...
  { id: 'avatar', name: 'Avatares Anim.', icon: Sparkles },
...
const getItemTypeLabel = (item) => {
  ...
  if (item.type === 'avatar') return 'Anm. Avatar';
...
const equipItem = async (item) => {
  try {
    ...
    if (item.type === 'avatar') {
      authStore.user.equipped_avatar_id = item.id;
      authStore.user.avatar_css = item.css_value;
    }
