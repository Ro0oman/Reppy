      ...
      >
        <div class="glass border-transparent rounded-[calc(1rem-2px)] p-4 flex items-center gap-4 relative z-10">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-0.5">
              {{ store.type === 'error' ? 'Security Protocol' : 'Neural Sync' }}
            </p>
            <p class="text-[13px] font-black text-white leading-tight">
              {{ store.message }}
            </p>
          </div>
          
          <button @click="store.hide" class="text-white/30 hover:text-white transition-colors p-1">
            <X class="w-4 h-4" />
          </button>
        </div>
