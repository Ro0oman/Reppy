<template>
  <div class="relative flex flex-col gap-4 shadow transition-opacity duration-500 "
    ...
    <div class="flex items-center justify-between px-1 ">
      ...
      <div class="flex items-center gap-2 bg-surface/50 p-1 rounded-xl border border-white/5">
        <button @click="prevMonth" :title="i18n.locale === 'es' ? 'Mes Anterior' : 'Previous Month'"
        ...
        <button @click="goToToday" :title="i18n.locale === 'es' ? 'Ir a hoy' : 'Go to today'"
        ...
        <button @click="nextMonth" :title="i18n.locale === 'es' ? 'Siguiente Mes' : 'Next Month'"
          ...
          <transition name="modal">
            <div v-if="selectedDay" class="fixed inset-0 z-[150] flex items-center justify-center p-6">
              ...
              <div
                ...
                <div
                  class="absolute -bottom-20 -left-20 w-40 h-40 bg-primary/5 blur-[80px] rounded-full pointer-events-none">
                </div>
                ...
                <div class="relative flex flex-col items-center gap-6">
                  ...
                  <div v-if="selectedDay.breakdown?.length > 0" class="w-full space-y-3">
                    <div class="flex items-center gap-2 px-1">
                      ...
                      <p class="text-[9px] font-black text-muted uppercase tracking-widest leading-none">Protocol
                        Breakdown</p>
                    </div>
                  ...
                  <button @click="selectedDay = null" :title="i18n.locale === 'es' ? 'Cerrar Detalle' : 'Close Detail'"
    ...
    <div class="flex items-center justify-end gap-3 px-1 opacity-60 hover:opacity-100 transition-opacity">
      ...
      <div class="flex gap-1.5">
        <div v-for="level in 4" :key="level" class="w-2.5 h-2.5 rounded-full" :class="getLevelClass(level)">
        </div>
      </div>
    </div>
  </div>
