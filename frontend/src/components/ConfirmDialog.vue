<template>
  <Transition name="modal">
    <div v-if="store.confirmVisible" class="fixed inset-0 z-[110] flex items-center justify-center px-4">
      ...
      <!-- Dialog -->
      <div class="glass w-full max-w-sm rounded-[2.5rem] p-8 border-white/10 relative z-10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden group">
        ...
        <div class="flex flex-col items-center text-center space-y-6 relative z-10">
          <div class="p-5 bg-zinc-900 rounded-3xl border border-white/5 shadow-2xl">
            <HelpCircle class="w-10 h-10 text-primary-500" />
          </div>
          
          <div class="space-y-2">
            <h3 class="text-xl font-black text-white uppercase tracking-tighter">{{ store.confirmTitle }}</h3>
          ...
          <div class="flex gap-3 w-full">
            <button 
              ...
              class="flex-1 py-4 bg-zinc-900 hover:bg-zinc-800 text-zinc-400 font-black uppercase tracking-widest text-[10px] rounded-2xl transition-all border border-white/5"
            >
              Cancel
            </button>
            <button 
