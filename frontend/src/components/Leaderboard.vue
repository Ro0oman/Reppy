<template>
  <div class="flex flex-col gap-6 p-2 sm:p-4 rounded-3xl overflow-hidden relative">
    ...
    <div class="flex flex-col gap-4 relative z-10 px-2">
      ...
      <div v-if="authStore.isAuthenticated" class="flex p-1 bg-steel-grey/40 rounded-xl border border-white/5 mx-auto w-full max-w-md">
        <button 
          @click="type = 'friends'"
          class="flex-1 py-2 text-[10px] font-black uppercase tracking-[0.2em] transition-all rounded-lg relative z-10"
          :class="type === 'friends' ? 'text-white' : 'text-zinc-500 hover:text-zinc-400'"
        >
          <span v-if="type === 'friends'" class="absolute inset-0 bg-white/5 border border-white/10 rounded-lg animate-scale-in"></span>
          FRIENDS
        </button>
        <button 
          @click="type = 'global'"
          class="flex-1 py-2 text-[10px] font-black uppercase tracking-[0.2em] transition-all rounded-lg relative z-10"
          :class="type === 'global' ? 'text-white' : 'text-zinc-500 hover:text-zinc-400'"
        >
          <span v-if="type === 'global'" class="absolute inset-0 bg-white/5 border border-white/10 rounded-lg animate-scale-in"></span>
          GLOBAL
        </button>
      </div>

      <!-- Timeframe Segmented Control (Industrial Dock) -->
      <div class="flex gap-1 p-1 bg-black/40 border border-white/5 rounded-xl overflow-x-auto no-scrollbar">
        <button 
        ...
        >
          <span v-if="timeframe === tf" class="absolute inset-0 bg-white/5 border border-white/5 rounded-lg animate-scale-in"></span>
          {{ tf }}
        </button>
      </div>
    </div>
    ...
    <div v-if="!loading && podiumUsers.length > 0" class="flex items-end justify-center gap-2 sm:gap-8 mt-12 mb-16 relative z-10">
      ...
      <div v-if="podiumUsers[1]" 
        ...
        <div class="relative mb-3">
          ...
          <div class="absolute -bottom-2 -right-1 w-6 h-6 bg-zinc-500 rounded-full flex items-center justify-center border-2 border-deep-abyss shadow-lg">
             <span class="text-[10px] font-black text-white">02</span>
          </div>
        </div>
        <p class="text-[10px] font-black text-zinc-400 uppercase tracking-tighter truncate w-20 text-center mb-0.5 font-tight">{{ podiumUsers[1].name }}</p>
        <span class="text-sm font-black text-white italic text-precision">{{ podiumUsers[1].total_reps }}</span>
      </div>
      ...
      <div v-if="podiumUsers[0]" 
        ...
        <div class="relative mb-4">
          ...
          <div class="absolute -bottom-2 -right-1 w-8 h-8 bg-primary-500 rounded-full flex items-center justify-center border-4 border-deep-abyss shadow-xl">
             <span class="text-xs font-black text-white">01</span>
          </div>
        </div>
        <p class="text-xs font-black text-white uppercase tracking-widest truncate w-28 text-center mb-0.5 leading-tight font-tight">{{ podiumUsers[0].name }}</p>
      ...
      <div v-if="podiumUsers[2]" 
        ...
        <div class="relative mb-3">
          ...
          <div class="absolute -bottom-2 -right-1 w-6 h-6 bg-orange-900 rounded-full flex items-center justify-center border-2 border-deep-abyss shadow-lg">
             <span class="text-[10px] font-black text-white">03</span>
          </div>
        </div>
        <p class="text-[10px] font-black text-zinc-400 uppercase tracking-tighter truncate w-20 text-center mb-0.5 font-tight">{{ podiumUsers[2].name }}</p>
        <span class="text-sm font-black text-white italic text-precision">{{ podiumUsers[2].total_reps }}</span>
      </div>
    </div>
    ...
    <div class="space-y-1 overflow-y-auto no-scrollbar scroll-smooth relative z-10 px-2 max-h-[500px]">
      <div 
        ...
        class="group flex items-center justify-between p-3 rounded-xl bg-white/[0.01] hover:bg-white/[0.03] transition-all duration-300 border border-transparent hover:border-white/5"
        :class="{'!bg-primary-500/5 !border-primary-500/20' : user.id === authStore.user?.id}"
      >
        <div class="flex items-center gap-4 min-w-0 flex-1">
          <!-- Position Badge -->
          <div class="w-10 flex-shrink-0">
            <span class="text-[11px] font-black text-zinc-700 tracking-tighter font-precision italic">#{{ String(index + 4).padStart(2, '0') }}</span>
          </div>
          ...
          <div class="relative flex-shrink-0 group-hover:scale-110 transition-transform">
            ...
            <div v-if="user.id === authStore.user?.id" class="absolute -top-1 -right-1 w-3 h-3 bg-primary-500 rounded-full border-2 border-deep-abyss z-10 animate-pulse"></div>
          </div>

          <!-- User Info -->
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <h4 class="text-sm font-black text-white truncate group-hover:text-primary-400 transition-colors tracking-tight uppercase italic leading-none font-tight">
                {{ user.id === authStore.user?.id ? 'YOU' : user.name }}
              </h4>
              <span v-if="user.title_name" class="hidden xs:inline-block text-[7px] uppercase tracking-widest px-2 py-0.5 rounded-md bg-white/5 border border-white/5 font-black shrink-0" :class="user.title_css">
                {{ user.title_name }}
              </span>
            </div>
        ...
        <div class="flex flex-col items-end leading-none translate-y-[2px] ml-4">
          <span class="text-xl font-black text-white group-hover:text-primary-500 transition-colors italic text-precision leading-none">
            {{ user.total_reps }}
          </span>
          <p class="text-[7px] font-black text-zinc-500 uppercase tracking-widest mt-1 opacity-60 leading-none">REPS</p>
        </div>
      </div>
      ...
      <div v-if="loading" class="py-24 flex flex-col items-center gap-4">
        ...
        <span class="text-[10px] font-black text-zinc-700 uppercase tracking-[0.4em]">SYNCING...</span>
      </div>
      
      <div v-if="!loading && users.length === 0" class="py-24 text-center opacity-30">
        <Users class="w-10 h-10 mx-auto mb-4 text-zinc-500" />
        <p class="text-[10px] font-black text-zinc-500 uppercase tracking-widest leading-none">PROTOCOL DATA NULL</p>
      </div>
    </div>
  </div>
</template>
