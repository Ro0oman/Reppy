  ...
  >
    <div v-if="show" class="fixed inset-0 z-[150] flex justify-center items-start overflow-y-auto p-4 md:p-8 bg-background/80 backdrop-blur-xl" @click.self="$emit('close')">
      <div class="glass max-w-xl w-full p-8 md:p-12 rounded-[2.5rem] border border-border shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-4 zoom-in-95 duration-300 my-auto">
        ...
        <!-- Close Button -->
        <button @click="$emit('close')" class="absolute top-8 right-8 p-3 bg-white/5 hover:bg-white/10 rounded-2xl transition-all group">
           <X class="w-6 h-6 text-muted group-hover:text-foreground" />
        </button>

        <!-- Header -->
        <div class="mb-10">
          <h2 class="text-3xl font-black italic tracking-tighter text-foreground uppercase leading-none">
            {{ i18n.t('settings_title') }}<span class="text-primary-500">.</span>MOD
          </h2>
          <p class="text-[10px] font-black text-muted uppercase tracking-[0.4em] mt-3">Interface & Protocol Override</p>
        </div>

        <div class="flex-1 space-y-8 overflow-y-auto pr-2 custom-scrollbar pb-4">
          <!-- Identity Section -->
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                <input v-model="form.name" type="text" class="input-tactical w-full" :placeholder="i18n.t('name_placeholder')" />
              </div>
              <div class="space-y-2">
                <label class="text-[8px] font-black uppercase text-muted tracking-widest px-1">{{ i18n.t('daily_goal') }}</label>
                <input v-model.number="form.daily_goal" type="number" class="input-tactical w-full" />
              </div>
            </div>
          </div>
        ...
        <!-- Footer Actions -->
        <div class="mt-10 pt-8 border-t border-border flex gap-4">
          <button @click="$emit('close')" class="flex-1 px-8 py-4 bg-surface/20 border border-border rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-surface/30 transition-all">
            {{ i18n.t('btn_cancel') }}
          </button>
          <button @click="save" :disabled="saving" class="btn-reppy flex-[2] !py-4 !text-[10px]">
