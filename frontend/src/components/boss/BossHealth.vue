<template>
  <div
    v-if="loading"
    class="animate-pulse bg-surface/30 h-72 rounded-2xl mb-8 border border-border/60"
  ></div>
  ...
  <div v-else-if="boss" class="space-y-5">
    ...
    >
      ...
      <header class="relative z-10 flex flex-wrap items-start justify-between gap-3">
        <div class="space-y-2">
          <div class="flex items-center gap-2 overflow-x-auto no-scrollbar whitespace-nowrap">
            <span
              class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wide border"
              :class="theme.badge"
            >
            ...
            <span
              ...
              class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wide border border-amber-500/30 bg-amber-500/10 text-amber-400"
            >
              <Shield class="w-3 h-3" />
              {{ weaknessLabel }} {{ weaknessDisplay }}
            </span>
          </div>
          <h3 class="text-2xl sm:text-3xl font-bold tracking-tight text-foreground leading-none">
            {{ boss.name }}
          </h3>
          <p v-if="boss.active_phrase && !isDefeated" class="text-xs sm:text-sm italic text-muted/90">
            "{{ boss.active_phrase }}"
          </p>
          <p v-if="isDefeated" class="inline-flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-emerald-400">
      ...
      <div class="relative z-10 mt-4 grid items-start grid-cols-[86px_minmax(0,1fr)] sm:grid-cols-[120px_minmax(0,1fr)] lg:grid-cols-[220px_minmax(0,1fr)] gap-3 sm:gap-4">
        ...
        <div class="rounded-2xl border border-border bg-black/20 p-3 sm:p-5 space-y-3 sm:space-y-4">
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <p class="text-xl sm:text-3xl font-bold tracking-tight text-foreground">
                ...
                <span class="text-xs font-bold text-muted/60">/ {{ formatNumber(boss.total_hp) }} HP</span>
              </p>
            </div>
            <div class="text-right">
              <p class="text-[10px] font-semibold uppercase tracking-wide text-muted/60">HP</p>
          ...
          <div class="grid grid-cols-3 gap-2" v-if="authStore.isAuthenticated">
            <div class="rounded-xl border border-border bg-foreground/[0.02] p-2.5">
              <p class="text-[10px] font-semibold uppercase tracking-wide text-muted/50">{{ statLabelPersonal }}</p>
              <p class="text-sm sm:text-lg font-bold tabular-nums text-foreground">{{ formatNumber(personalDamage) }}</p>
            </div>
            <div class="rounded-xl border border-border bg-foreground/[0.02] p-2.5">
              <p class="text-[10px] font-semibold uppercase tracking-wide text-muted/50">{{ statLabelToday }}</p>
              <p class="text-sm sm:text-lg font-bold tabular-nums text-emerald-400">{{ formatNumber(dailyDamage) }}</p>
            </div>
            <div class="rounded-xl border border-border bg-foreground/[0.02] p-2.5">
              <p class="text-[10px] font-semibold uppercase tracking-wide text-muted/50">{{ statLabelTop }}</p>
              <p class="text-[11px] sm:text-sm font-bold tabular-nums text-amber-400 truncate" :title="topDealerName">{{ topDealerName }}</p>
            </div>
          </div>
        </div>
      </div>

      <div class="relative z-10 mt-3 sm:mt-4 rounded-2xl border border-border bg-black/20 p-3 sm:p-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="hidden sm:flex flex-wrap gap-2">
            <span
              ...
              :key="rate.name"
              class="px-2.5 py-1 rounded-lg border border-border bg-foreground/[0.03] text-[10px] font-semibold uppercase tracking-tight text-foreground/90"
            >
        ...
        <div class="mt-4">
          ...
          <div
            ...
            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-border bg-foreground/[0.03] text-[10px] font-semibold uppercase tracking-wide text-muted"
          >
          ...
          <div
            ...
            class="text-[10px] font-semibold uppercase tracking-wide text-primary-400/90"
          >
            {{ i18nStore.t('boss_login_to_damage') }}
          </div>
          <div
            v-else
            class="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wide text-muted/70"
          >
    ...
    <div
      v-if="showHelp"
      class="cursor-pointer fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm"
      @click.self="showHelp = false"
    >
      <div class="w-full max-w-2xl rounded-xl border border-border bg-surface/30 p-6 sm:p-8">
        <div class="flex items-start justify-between gap-4">
          <div>
            <p class="text-[10px] font-semibold uppercase tracking-wide text-primary-500">{{ i18nStore.t('boss_op_manual') }}</p>
            <h4 class="mt-2 text-2xl font-bold tracking-tight text-foreground">{{ i18nStore.t('boss_community_event') }}</h4>
          </div>
          <button
            @click="showHelp = false"
            class="w-9 h-9 rounded-xl border border-border bg-foreground/[0.04] text-muted hover:text-foreground"
          >
            <XIcon class="w-4 h-4 mx-auto" />
          </button>
        </div>
        <div class="mt-5 space-y-3">
          <div class="rounded-xl border border-border bg-black/25 p-3">
            <p class="text-[10px] font-semibold uppercase tracking-wide text-foreground">{{ i18nStore.t('battle_help_title1') }}</p>
            <p class="mt-1 text-xs text-muted">{{ i18nStore.t('battle_help_desc1') }}</p>
          </div>
          <div class="rounded-xl border border-border bg-black/25 p-3">
            <p class="text-[10px] font-semibold uppercase tracking-wide text-foreground">{{ i18nStore.t('battle_help_title2') }}</p>
          ...
          <div class="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-3">
            <p class="text-[10px] font-semibold uppercase tracking-wide text-emerald-400">{{ i18nStore.t('boss_reward_chest') }}</p>
