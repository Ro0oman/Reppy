...
- [ ] 🔨 PR #325 **Cap a la recompensa de racha** ✅DECIDIDO (2026-07-25: opción A) — `backend/utils/stats.js:300`: cambiar `streak × 50` por **`50 + 5×min(streak,30)`** (cap 200 monedas/día). Verificar que no rompe ningún otro cálculo que dependa de la recompensa de racha.
...
- [ ] ⏸️ **Arreglar la curva de dificultad** ✅DECIDIDO (2026-07-25: opción A) — que el HP enemigo escale con el **daño esperado del jugador** (cuadrático en nivel), no `1+0.05·L` lineal, para que las reps-para-matar se mantengan estables al avanzar en vez de caer 41→0.01. Requiere re-derivar la fórmula de HP contra la de daño (usar `docs/economy-curves-2026-07.md` de PR #314 como base). Diseño delicado → PR revisable.
...
- [ ] ⏸️ **Prestigio con ROI positivo** ✅DECIDIDO (2026-07-25: opción B) — no subir `rewards.prestige_mult` (evitar inflación de monedas); en su lugar dar **recompensas exclusivas cosméticas** (títulos/marcos/efectos exclusivos de prestigio) por cada vuelta de NG+. Definir qué cosméticos y engancharlos al evento de prestigio. Feature de contenido → PR revisable.
...
- [ ] **Combate: ejercicio → daño vs weakness/resist** ✅DECIDIDO (2026-07-25: opción B) — MANTENER el bono por nivel de stat actual Y AÑADIR un bono/penalización extra según el ejercicio logueado: si el stat del ejercicio coincide con `boss.weakness_stat` → bonus; si coincide con `resist_stat` → penalización (esto además ACTIVA `resist_stat`, hoy muerto). Toca `backend/utils/damage.js:130-137`. ⚠️ FALTA que Roman pase la tabla **ejercicio→tipo-de-daño** y los **multiplicadores** antes de implementar — NO implementar a ciegas.
...
- [ ] 🔨 PR #330 (comentario; `HEVY_ENCRYPTION_KEY` sigue 👤 infra) **SSL de db.js: documentar y cerrar** ✅DECIDIDO (2026-07-25: opción B) — dejar `rejectUnauthorized:false` (la conexión a Supabase ya va por TLS; solo no se valida el CA — riesgo bajo). Añadir comentario en `backend/db.js:13` explicando el porqué. ADEMÁS (caveat del agente): fijar `HEVY_ENCRYPTION_KEY` en Coolify (hoy cae a `JWT_SECRET`/default de dev). La Hevy key ya se cifra AES-256-GCM ✓. Accionable ya.
- [ ] 🔨 PR #331 **Borrar shim `api/` + vercel.json** ✅DECIDIDO (2026-07-25: opción B) — borrar el directorio `api/` (shim Vercel muerto, ya estáis en Coolify) y `vercel.json`. ⚠️ NO TOCAR el doble montaje de `apiRouter` en `/` y `/api` (`index.js:293-294`): es LOAD-BEARING (Traefik quita el prefijo /api; quitar el mount `/` tira producción). Accionable ya.
- [x] **`social_feed.js:318`** (PR #319 mergeada) — `page` no numérico → 500 en vez de 400.
- [ ] 🔨 PR #332 (dark) · ⏸️ (light) **Sendas de campaña: implementar efectos** ✅DECIDIDO (2026-07-25: opción A) — implementar los efectos numéricos hoy muertos: `dark → curse_damage_multiplier` (aplicar al daño de combate de campaña) y `light → blessing_hours` (buff temporal al elegir senda). Toca `campaignEngine.js`. ⚠️ FALTA que Roman confirme los números (el JSON trae dark 0.75 / light 24h — usar esos o ajustar) — es balance, no implementar a ciegas los multiplicadores sin su OK.
  ...
  - **Parte `light` ⏸️ PREGUNTA (agente 2026-07-26)**: el config solo define `blessing_hours: 24` = **duración**, NO qué hace la bendición. Para implementarla hay que definir el EFECTO: ¿un `damage_multiplier` temporal (¿de cuánto? p. ej. ×1.15 durante 24h, usando el mecanismo existente `users.damage_multiplier`/`damage_multiplier_expiry`)? ¿o algo de recompensa (coherente con `reward_variance: low`)? Sin ese dato tendría que inventar el multiplicador. **Roman define el efecto+magnitud de la bendición y añade el campo al JSON (p. ej. `light.blessing_damage_mult`) → se implementa.**
