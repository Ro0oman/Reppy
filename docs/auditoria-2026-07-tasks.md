...
- [ ] **Borrar shim `api/` + vercel.json** ✅DECIDIDO (2026-07-25: opción B) — borrar el directorio `api/` (shim Vercel muerto, ya estáis en Coolify) y `vercel.json`. ⚠️ NO TOCAR el doble montaje de `apiRouter` en `/` y `/api` (`index.js:293-294`): es LOAD-BEARING (Traefik quita el prefijo /api; quitar el mount `/` tira producción). Accionable ya.
