...
- [x] (dark) · ⏸️ (light) **Sendas de campaña: implementar efectos** ✅DECIDIDO (2026-07-25: opción A) — implementar los efectos numéricos hoy muertos: `dark → curse_damage_multiplier` (aplicar al daño de combate de campaña) y `light → blessing_hours` (buff temporal al elegir senda). Toca `campaignEngine.js`. ⚠️ FALTA que Roman confirme los números (el JSON trae dark 0.75 / light 24h — usar esos o ajustar) — es balance, no implementar a ciegas los multiplicadores sin su OK.
