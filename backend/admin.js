  ...
  try {
    ...
    if (itemRes.rows.length === 0) {
      const COSMIC = [
        ...
        { name: 'Yelmo del Vacío',    type: 'head',   css_value: null, svg_key: 'helmet_void',  price: 15000, price_gems: 300, stats: JSON.stringify({ str: 24, pwr: 20, end: 24, agi: 16 }),  description: 'CÓSMICO · El vacío entre galaxias comprimido en acero estelar. No existe resistencia que no doble ante este yelmo.' },
        { name: 'Filo Estelar',       type: 'weapon', css_value: null, svg_key: 'sword_cosmic', price: 15000, price_gems: 300, stats: JSON.stringify({ str: 30, pwr: 28, crit_chance: 16, crit_damage: 60 }), description: 'CÓSMICO · Forjado en el núcleo de una estrella de neutrones. Cada rep es una explosión de energía cósmica.' },
        { name: 'Coraza Nebulosa',    type: 'armor',  css_value: null, svg_key: 'armor_cosmic', price: 15000, price_gems: 300, stats: JSON.stringify({ end: 28, pwr: 20, str: 20, agi: 16 }),  description: 'CÓSMICO · Placas forjadas con materia oscura. Absorbe el dolor como un agujero negro absorbe la luz.' },
        { name: 'Grebas del Cosmos',  type: 'boots',  css_value: null, svg_key: 'boots_cosmic', price: 15000, price_gems: 300, stats: JSON.stringify({ agi: 28, end: 20, str: 16, pwr: 20 }), description: 'CÓSMICO · La velocidad de la luz hecha calzado. Cada paso resuena en el tejido del espacio-tiempo.' },
      ];
...
router.post('/send-referral-reminder', authenticate, isAdmin, async (req, res) => {
  try {
    const count = await runReferralReminders();
    res.json({ message: `Referral reminder enviado a ${count} usuarios.` });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
